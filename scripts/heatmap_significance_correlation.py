...
def compute_correlations_for_clusters(df, rendement_var, groupes_variables):
    """Computes correlation matrices with significance levels for each cluster."""
...
def annotate_correlation_matrix_with_significance(correlations, significance):
    """Annotates correlation matrix with significance stars."""
...
def plot_heatmap(df, title, output_path):
    ...
    sns.heatmap(df.astype(float), annot=True, fmt="", cmap="RdYlGn", cbar=True, annot_kws={"size": 32}, linewidths=0.5, linecolor="black")
...
def main():
    folder = '/Users/sophiegrosse/Documents/Greenresilience/Doctorat/Database/Irrigation/'
    df1 = load_and_label_clusters(os.path.join(folder, 'ClusterA/clusteringA2.xlsx'), 'R')
    df2 = load_and_label_clusters(os.path.join(folder, 'ClusterB/clusteringB2.xlsx'), 'MI')
    df3 = load_and_label_clusters(os.path.join(folder, 'ClusterC/clusteringC.xlsx'), 'HI')
...
'SPI_SPEI_sepnov': ['SPI_9', 'SPI_10', 'SPI_11', 'SPI_91011', 'SPEI_9', 'SPEI_10', 'SPEI_11', 'SPEI_91011'],
    ...
    correlations_by_cluster, significance_by_cluster = compute_correlations_for_clusters(df_combined, rendement_var, groupes_variables)
    ...
    heatmap_path = os.path.join(folder, "heatmap_with_significance.png")
    plot_heatmap(annotated_matrix, "Heatmap of Correlations with Significance", heatmap_path)
